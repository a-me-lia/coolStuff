        ...
        <div className="flex flex-col font-mono text-[16px] w-full bg-white">
          <div className="flex flex-row items-baseline">
            {Object.entries(navItems).map(([path, { name, id }]) => {
                const isActive = path === pathname;
              return(
                <Link key={path} href={path}  className={`pr-10 ${isActive ? 'text-neutral-900' : 'text-neutral-400' } transition-colors duration-1000`}><p id={id}>{name}</p></Link>
              );
            })}
          </div>
